export const pdfOrganizeContent: Record<string, { sections: { title: string, content: string }[] }> = {
  "merge-pdf": {
    sections: [
      ...
      {
        title: "Zero-Retention Privacy Promise: Strict Ephemeral Security for Word & PDF Files",
        content: "<p>Most online file converters store your sensitive business contracts, medical forms, tax returns, and personal resumes on remote servers. At <strong>ConverterForAll</strong>, we adhere to a strict <strong>Zero-Retention Architecture</strong>. Standard PDF operations (merging, splitting, rotating, organizing) execute 100% locally on your device via client-side WebAssembly. When complex Word documents (.docx) are converted, processing occurs in isolated, volatile memory (RAM) and files are <strong>immediately destroyed after download</strong>. Zero data is logged, shared, or permanently saved on disk, ensuring total GDPR and privacy compliance.</p>"
      },
      {
      ...
      },
      {
        ...
        content: `
          <p><strong>Q: Can I merge Microsoft Word (.docx / .doc) files directly with PDF documents?</strong><br>A: Yes! You can upload Word documents alongside existing PDFs. Our tool automatically converts the Word document into crisp PDF pages and merges them together seamlessly into one final document.</p>
          ...
          <p><strong>Q: Are my files kept private and secure?</strong><br>A: 100% yes. We operate on a strict Zero-Retention Privacy Promise. Documents are processed either on-device or in temporary ephemeral memory and are immediately purged after download. Your private files are never permanently stored, logged, or shared with third parties.</p>
